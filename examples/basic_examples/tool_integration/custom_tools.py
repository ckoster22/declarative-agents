...
def calculator_tool(expression: str) -> str:
    ...
    try:
        ...
        return json.dumps({"error": "Division by zero is not allowed", "expression": expression})
    except Exception as e:
        return json.dumps({"error": f"Error evaluating expression: {str(e)}", "expression": expression})
...
def weather_lookup_tool(city: str) -> str:
    ...
    if city in weather_data:
        ...
        return json.dumps(
            {"error": f"Weather data not available for {city}", "available_cities": list(weather_data.keys())}
        )
